  ...
  function setInit() {
    ...
    if (themeConfig['copyright']) {
        ...
        if (themeConfig['copyright']['text']) {
          ...
          text = `<span>` + `Copyright © ${year} ${themeConfig['copyright']['author']} ${i18n['ReservedRights']}` + `</span>`;
        }
        $('.page-inner .copyright').append(text);
        if(themeConfig['copyright']['time']) {
          let time = now;
          let tagText = `<span>最后修改于` + `${time.getFullYear().toString().padStart(4, '0')}-${(time.getMonth() + 1).toString().padStart(2, '0')}-${time.getDate().toString().padStart(2, '0')} ${time.getHours().toString().padStart(2, '0')}:${time.getMinutes().toString().padStart(2, '0')}:${time.getSeconds().toString().padStart(2, '0')}` + `</span>`;
          $('.page-inner .copyright').append(tagText);
        }
    }
  }
